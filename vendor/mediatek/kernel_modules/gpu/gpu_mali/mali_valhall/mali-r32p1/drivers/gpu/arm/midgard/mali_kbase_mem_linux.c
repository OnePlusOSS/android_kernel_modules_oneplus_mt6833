...
static void kbase_csf_user_reg_vm_close(struct vm_area_struct *vma)
{
	...
	if (unlikely(!kctx->csf.user_reg.vma))
		dev_warn(kctx->kbdev->dev, "user_reg vma pointer unexpectedly NULL");

	mutex_lock(&kctx->kbdev->csf.reg_lock);
	if (unlikely(list_empty(&kctx->csf.user_reg.link)))
		dev_warn(kctx->kbdev->dev, "The User reg link node isn't in the list");
	else
...
static vm_fault_t kbase_csf_user_reg_vm_fault(struct vm_fault *vmf)
{
	...
	if (WARN_ON(nr_pages != 1) ||
	    WARN_ON(vma != kctx->csf.user_reg.vma) ||
	...
	if (!kbdev->pm.backend.gpu_powered) {
		...
		WARN_ON(kctx->csf.user_reg.dummy_page_mapped);
		kctx->csf.user_reg.dummy_page_mapped = true;
	} else {
		WARN_ON(kctx->csf.user_reg.real_page_mapped);
		kctx->csf.user_reg.real_page_mapped = true;
	}
				...
				struct vm_area_struct *vma)
{
	 ...
	 * vma is closed).
	 */
	vma->vm_file = kctx->csf.user_reg.filp;
	get_file(vma->vm_file);
	list_add_tail(&kctx->csf.user_reg.link, &kbdev->csf.user_reg.list);
	mutex_unlock(&kbdev->csf.reg_lock);
