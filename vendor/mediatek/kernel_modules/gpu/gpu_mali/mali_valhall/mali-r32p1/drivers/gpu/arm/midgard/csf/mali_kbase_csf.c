...
static void ctx_user_reg_page_mapping_cleanup(struct kbase_context *kctx)
{
	WARN_ON(kctx->csf.user_reg.vma);
	WARN_ON(!list_empty(&kctx->csf.user_reg.link));

	fput(kctx->csf.user_reg.filp);
}

static int ctx_user_reg_page_mapping_setup(struct kbase_context *kctx)
{
	char name[9 + 2 + 2 * 10];
	struct file *filp;

	INIT_LIST_HEAD(&kctx->csf.user_reg.link);
	kctx->csf.user_reg.dummy_page_mapped = false;
	kctx->csf.user_reg.real_page_mapped = false;
	kctx->csf.user_reg.vma = NULL;

	snprintf(name, sizeof(name), "user_reg_%d_%d", kctx->tgid, kctx->id);
	filp = shmem_file_setup(name, MAX_LFS_FILESIZE, VM_NORESERVE);
	if (IS_ERR(filp))
		return PTR_ERR(filp);

	kctx->csf.user_reg.filp = filp;
...
static void pending_submission_worker(struct work_struct *work)
{
	...
	if (err) {
		dev_vdbg(kbdev->dev, "Unsuccessful GPU reset detected when kicking queue ");
		return;
	}
		...
		if (likely(!err)) {
			...
			if (likely(!err)) {
				...
				if (likely(!err)) {
					...
					err = ctx_user_reg_page_mapping_setup(kctx);
					if (unlikely(err))
...
void kbase_csf_ctx_term(struct kbase_context *kctx)
{
	...
	ctx_user_reg_page_mapping_cleanup(kctx);
	kbase_csf_tiler_heap_context_term(kctx);
	kbase_csf_kcpu_queue_context_term(kctx);
	kbase_csf_scheduler_context_term(kctx);
		...
		if (queue && !WARN_ON(queue->csi_index != i)) {
			...
			if (((cs_req & CS_REQ_TILER_OOM_MASK) ^
				...
				if (WARN_ON(!queue_work(wq, &queue->oom_event_work))) {
					...
					ged_log_buf_print2(
						kbdev->ged_log_buf_hnd_kbase, GED_LOG_ATTR_TIME,
						"OoM event have been already queued\n");
#endif
					release_queue(queue);
				}
			}
...
{
	if (as_phys_addr_t(kbdev->csf.user_reg.dummy_page)) {
...
int kbase_csf_setup_dummy_user_reg_page(struct kbase_device *kbdev)
{
	...
	kbdev->csf.user_reg.dummy_page = as_tagged(0);
