...
static void kbasep_ctx_user_reg_page_mapping_term(struct kbase_context *kctx)
{
	...
	if (unlikely(kctx->csf.user_reg.vma))
		dev_err(kbdev->dev,
			"VMA for USER Register page exist on termination of ctx %d_%d",
			kctx->tgid, kctx->id);
...
int kbase_csf_setup_dummy_user_reg_page(struct kbase_device *kbdev)
{
	...
	if (IS_ERR(filp)) {
		dev_err(kbdev->dev,
			"failed to get an unlinked file for user_reg");
		return PTR_ERR(filp);
	}

	if (kbase_mem_pool_alloc_pages(
	...
	kbase_sync_single_for_device(kbdev, kbase_dma_addr(page) + LATEST_FLUSH,
