					...
					int next_profile, int vstep)
{
	...
	if (ftm_mode == BOOT_MODE_FACTORY) {
		...
		if (!ready) {
			memset(ftm_clk, 0x00, sizeof(ftm_clk));
			strncpy(ftm_clk, "clk_fail", sizeof(ftm_clk) - 1);
		}
	}
	#endif /* OPLUS_BUG_COMPATIBILITY */
	return err;
...
static const struct file_operations tfa98xx_dbgfs_r_tfa_cali_sq_fops = {
	.open = simple_open,
	.read = tfa98xx_dbgfs_r_tfa_cali_sq_read,
	.llseek = default_llseek,
};
#endif /* OPLUS_ARCH_EXTENDS */
...
static int get_profile_from_list(char *buf, int id)
{
	...
	list_for_each_entry(bprof, &profile_list, list) {
		if (bprof->item_id == id) {
			memset(buf, 0x00, sizeof(MAX_CONTROL_NAME));
			strncpy(buf, bprof->basename, MAX_CONTROL_NAME - 1);
			return err;
		}
	}
	err = -1;
			 ...
			 struct snd_ctl_elem_info *uinfo)
{
	...
	memset(uinfo->value.enumerated.name, 0x00, sizeof(uinfo->value.enumerated.name));
	strncpy(uinfo->value.enumerated.name, profile_name, sizeof(uinfo->value.enumerated.name) - 1);
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	if (tfa98xx_container == NULL) {
		...
		if (tfa_err != tfa_error_ok) {
			#ifdef OPLUS_BUG_COMPATIBILITY
			memset(ftm_load_file, 0x00, sizeof(ftm_load_file));
			strncpy(ftm_load_file, "load_file_fail", sizeof(ftm_load_file) - 1);
			#endif /* OPLUS_BUG_COMPATIBILITY */
			//mutex_unlock(&tfa98xx_mutex);
			kfree(container);
			dev_err(tfa98xx->dev,
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	if (failed) {
		...
		#ifdef OPLUS_BUG_COMPATIBILITY
		if (ftm_mode == BOOT_MODE_FACTORY) {
			memset(ftm_path, 0x00, sizeof(ftm_path));
			strncpy(ftm_path, "open_path_fail", sizeof(ftm_path) - 1);
		}
		#endif /* OPLUS_BUG_COMPATIBILITY */
	}
	mutex_unlock(&tfa98xx->dsp_lock);
	...
	if (sync) {
		...
		if (ftm_mode == BOOT_MODE_FACTORY) {
			...
			memset(ftm_tfa98xx_flag, 0x00, sizeof(ftm_tfa98xx_flag));
			strncpy(ftm_tfa98xx_flag, "ok", sizeof(ftm_tfa98xx_flag) - 1);
		}
		#endif /* OPLUS_BUG_COMPATIBILITY */
	}
}
						...
						struct snd_soc_dai *dai)
{
	...
	#ifdef OPLUS_ARCH_EXTENDS
	//fix coverity 30400
	profile_name = tfa_cont_profile_name(tfa98xx, tfa98xx->profile);
	if (NULL != profile_name) {
		get_profile_basename(basename, profile_name);
	}
	#else
	...
	for (prof = 0; prof < nprof; prof++) {
		#ifdef OPLUS_ARCH_EXTENDS
		//fix coverity 30400
		profile_name = tfa_cont_profile_name(tfa98xx, prof);
		if ((NULL != profile_name)&&(strncmp(basename, profile_name, len) == 0)) {
		#else
		if (strncmp(basename,
				...
				const struct i2c_device_id *id)
{
	...
	snprintf(buf, sizeof(buf), "tfa98xx.%x-%04x", i2cbus, addr);
