...
static void goodix_game_bit_set(struct chip_data_brl *chip_info, unsigned int bit, bool enable);
...
static int goodix_enable_game_mode(struct chip_data_brl *chip_info, bool enable)
{
	...
	if (enable) {
		...
		goodix_game_bit_set(chip_info, GAME_MODE_ENABLE, true);
		...
		goodix_game_bit_set(chip_info, GAME_MODE_ENABLE, false);
		goodix_state_verify(chip_info);
	}
...
{
	TPD_INFO("GT:%s, pen control cmd is [%2d]-[0x%x]\n", __func__, ctl_cmd, ctl_cmd);

	if (ctl_cmd == PEN_CTL_FEEDBACK) {
		TPD_INFO("GT:%s flag is feedback, return now control para\n", __func__);
		goto REPORT;
	}

	if (!ctl_cmd) {
		TPD_INFO("GT:%s invaled data!!\n", __func__);
		goto ERR;
	}

	if (ctl_cmd & PEN_CTL_VIBRATOR) {
		TPD_INFO("GT:%s find vibrator cmd and set cmd:%d\n", __func__, ctl_cmd & PEN_CTL_VIBRATOR_ENABLE);
		if (goodix_send_cmd_simple(chip_info, PEN_CMD_VIBRATOR, !(ctl_cmd & PEN_CTL_VIBRATOR_ENABLE)) < 0) {
			TPD_INFO("GT:%s fail to set vibrator cmd:%d\n", __func__, ctl_cmd & PEN_CTL_VIBRATOR_ENABLE);
			goto ERR;
		}
	}
...
static int get_now_temp(struct chip_data_brl *chip_info)
{
	...
	if (rc < 0) {
		TPD_INFO("%s : can't get skin temp, rc=%d\n", __func__, rc);
		return rc;
	}
			...
			struct point_info *points, int max_num)
{
	...
	for (i = 0; i < touch_num; i++) {
		...
		TPD_DEBUG("GT:[TP]:[%s]:points[%d](%6d,%6d).tx_press[%2d].rx_press[%2d].tx_er[%2d].rx_er[%2d]",
				points[id].x > (chip_info->max_y / 2 ) ? "right" : "left",
				id, points[id].x, points[id].y,
				points[id].tx_press, points[id].rx_press, points[id].tx_er, points[id].rx_er);
...
static void goodix_get_pen_points(void *chip_data, struct pen_info *pen_info)
{
	...
	TPD_DEBUG("GT:%s:check pen data = [%*ph]\n", __func__,
		IRQ_EVENT_HEAD_LEN, chip_info->touch_data);
...
static void goodix_game_bit_chg(struct chip_data_brl *chip_info, unsigned int bit)
{
	if (((BIT_CHK(chip_info->game_old_state, bit) && !BIT_CHK(chip_info->game_now_state, bit)) ||
		(!BIT_CHK(chip_info->game_old_state, bit) && BIT_CHK(chip_info->game_now_state, bit))) &&
		!BIT_CHK(chip_info->game_chg_state, bit)) {
		chip_info->game_chg_state = BIT_SET(bit) | chip_info->game_chg_state;
	} else if (((!BIT_CHK(chip_info->game_old_state, bit) && !BIT_CHK(chip_info->game_now_state, bit)) ||
				(BIT_CHK(chip_info->game_old_state, bit) && BIT_CHK(chip_info->game_now_state, bit))) &&
				BIT_CHK(chip_info->game_chg_state, bit)) {
		chip_info->game_chg_state = BIT_CLR(bit) & chip_info->game_chg_state;
	}
	TPD_DEBUG("GT: %s-->[old:%d]-[now:%d]-[chg:%d]\n",
		__func__,
		chip_info->game_old_state, chip_info->game_now_state, chip_info->game_chg_state);
}

static void goodix_game_bit_set(struct chip_data_brl *chip_info, unsigned int bit, bool enable)
{
	if (enable == true) {
		chip_info->game_now_state = BIT_SET(bit) | chip_info->game_now_state;
		goodix_game_bit_chg(chip_info, bit);
		chip_info->game_old_state = BIT_SET(bit) | chip_info->game_now_state;
	} else {
		chip_info->game_now_state = BIT_CLR(bit) & chip_info->game_now_state;
		goodix_game_bit_chg(chip_info, bit);
		chip_info->game_old_state = BIT_CLR(bit) & chip_info->game_now_state;
	}
}
...
static void goodix_state_verify(struct chip_data_brl *chip_info)
{
	...
	if (chip_info->game_chg_state &&
		chip_info->pen_enable && chip_info->pen_support &&
		!ts->is_suspended) {
		if (BIT_CHK(chip_info->game_now_state, GAME_MODE_ENABLE)) {
			TPD_INFO("GT: %s->game on->must disable pen\n", __func__);
			ret = goodix_enable_pen_mode(chip_info, false);
		} else if (!BIT_CHK(chip_info->game_now_state, GAME_MODE_ENABLE)) {
			...
			ret = goodix_pen_control(chip_info, chip_info->pen_ctl_para);
		}
	}
}
				   ...
				   void *chip_data, debug_type debug_type)
{
	...
	msleep(20);
	ret = goodix_reg_write(chip_info, ic_info->misc.touch_data_addr, &clear_state, 1);

	while (i++ < 10) {
		ret = goodix_reg_read(chip_info, ic_info->misc.touch_data_addr, kernel_buf, 1);
		TPD_INFO("GT:ret = %d  kernel_buf = %d\n", ret, kernel_buf[0]);
...
static void init_goodix_chip_dts(struct device *dev, void *chip_data)
{
	...
	rc = of_property_read_bool(np, "temperature_detect_support");

	TPD_INFO("GT:%s : temp is %d", __func__, rc);
...
static int goodix_gt9966_ts_probe(struct spi_device *spi)
{
	...
	chip_info->game_now_state =       0;
	chip_info->game_old_state =       0;
	chip_info->game_chg_state =       0;
