...
#include <../sched/frame_boost/frame_info.h>
...
static void fbg_update_ed_task_info(unsigned int type)
{
	...
	case BOOST_ED_TASK_MID_DURATION:
		update_ed_task_boost_mid_duration(fbg_get_stune_boost(BOOST_ED_TASK_MID_DURATION));
		break;
	case BOOST_ED_TASK_MID_UTIL:
		ed_task_boost_mid_util = fbg_get_stune_boost(BOOST_ED_TASK_MID_UTIL);
		break;
	case BOOST_ED_TASK_MAX_DURATION:
		update_ed_task_boost_max_duration(fbg_get_stune_boost(BOOST_ED_TASK_MAX_DURATION));
		break;
	case BOOST_ED_TASK_MAX_UTIL:
		ed_task_boost_max_util = fbg_get_stune_boost(BOOST_ED_TASK_MAX_UTIL);
		break;
	case BOOST_ED_TASK_TIME_OUT_DURATION:
		update_ed_task_boost_timeout_duration(fbg_get_stune_boost(BOOST_ED_TASK_TIME_OUT_DURATION));
		break;
	}
}
	...
	case CMD_ID_BOOST_HIT:
		...
		if (data.stage == BOOST_OBTAIN_VIEW) {
			if (get_frame_rate() >= fbg_get_stune_boost(BOOST_UTIL_FRAME_RATE)) {
				set_frame_util_min(
						fbg_get_stune_boost(BOOST_UTIL_MIN_OBTAIN_VIEW), true);
				default_group_update_cpufreq();
			}
		}
		...
		if (data.stage == BOOST_FRAME_TIMEOUT) {
			if (get_frame_rate() >= fbg_get_stune_boost(BOOST_UTIL_FRAME_RATE) &&
					check_putil_over_thresh(fbg_get_stune_boost(BOOST_UTIL_MIN_THRESHOLD))) {
...
static long ofb_sys_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case CMD_ID_BOOST_STUNE: {
		...
		if (SYSTEM_UID == uid) {
			...
			if (stune_data.boost_freq == 0 && stune_data.boost_migr == 0 && stune_data.vutil_margin == 0) {
				/* restore to last_sys_stune_data */
				setup_stune_data(&last_sys_stune_data);
				break;
			}
			/* just return if current stune is boosted */
			if (stune_data.boost_freq <= last_sys_stune_data.boost_freq && stune_data.boost_migr <= last_sys_stune_data.boost_migr) {
				break;
			}
		}
		...
		size_t count, loff_t *ppos)
{
	...
	while ((token = strsep(&temp_str, " ")) && *token && (i < BOOST_MAX_TYPE)) {
		...
		if (boost_val >= 0 && i < BOOST_MAX_TYPE) {
			...
			} else if ((i == BOOST_UTIL_MIN_THRESHOLD) ||
					(i == BOOST_UTIL_MIN_OBTAIN_VIEW) ||
					(i == BOOST_UTIL_MIN_TIMEOUT)) {
