...
{
	return (sysctl_sched_impt_tgid != 0 && tsk->tgid == sysctl_sched_impt_tgid); //audio third party group write by oplus audio team
}
	...
	case TASK_SMALL:
		if ((tsk->oplus_task_info.sa_info[TST_EXEC][TASK_INFO_SAMPLE-1] != 0) && (tsk->oplus_task_info.sa_info[TST_SLEEP][TASK_INFO_SAMPLE-1] != 0)) {
			while (i < TASK_INFO_SAMPLE) {
				if ((tsk->oplus_task_info.sa_info[TST_EXEC][i] < RUNNING_MIN_THREASH) && ((tsk->oplus_task_info.sa_info[TST_EXEC][i] * 5) < tsk->oplus_task_info.sa_info[TST_SLEEP][i])) {
					fit_small++;
				}
