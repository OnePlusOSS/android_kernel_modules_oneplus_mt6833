...
bool _slide_task_misfit(struct task_struct *p, int cpu)
{
	...
	return (is_task_util_over(p, sysctl_boost_task_threshold)&& cpu < num_mincpu);
}
...
u64 _slide_get_boost_load(int cpu) {
	...
	if ((sched_assist_scene(SA_SLIDE) || sched_assist_scene(SA_INPUT)|| sched_assist_scene(SA_ANIM)) && ux_task_load[cpu]) {
...
/* we may need to always keep slide-scene, use it! */
bool __weak keep_slide_nolimit(struct task_struct *p)
{
	return false;
}
...
void slide_calc_boost_load(struct rq *rq, unsigned int *flag, int cpu) {
	...
	if (keep_slide_nolimit(rq->curr) || sched_assist_scene(SA_SLIDE) || sched_assist_scene(SA_INPUT) || sched_assist_scene(SA_ANIM)) {
		if(rq->curr && (is_heavy_ux_task(rq->curr) || rq->curr->sched_class == &rt_sched_class) && !_slide_task_misfit(rq->curr, rq->cpu)) {
...
bool should_adjust_slide_task_placement(struct task_struct *p, int cpu)
{
	...
	if (!check_slide_scene() && !keep_slide_nolimit(p))
		return false;
